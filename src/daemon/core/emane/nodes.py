...
class EmaneNode(EmaneNet):
    ...
    def netifs(self, sort=True):
        ...
        '''
        return sorted(list(self._netif.values()), key=lambda ifc: ifc.node.objid)
        ...
        for netif in moved_netifs:
            ...
            if self.verbose:
                self.info("setnempositions %d %s (%s) x,y,z=(%d,%d,%s)"
                          "(%.6f,%.6f,%.6f)" % \
                          (i, ifname, nemid, x, y, z, lat, int, alt))
            # altitude must be an integer or warning is printed
            alt = int(round(alt))
            event.set(i, nemid, lat, int, alt)
            i += 1
