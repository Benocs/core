...
def createclients(sessiondir, clientcls = VnodeClient,
    ...
    direntries = [os.path.join(sessiondir, x) for x in os.listdir(sessiondir)]
    cmdchnls = [x for x in direntries if stat.S_ISSOCK(os.stat(x).st_mode)]
    if cmdchnlfilterfunc:
        cmdchnls = list(filter(cmdchnlfilterfunc, cmdchnls))
    cmdchnls.sort()
    return [clientcls(os.path.basename(x), x) for x in cmdchnls]
...
def createremoteclients(sessiondir, clientcls = VnodeClient,
    ...
    '''
    direntries = [os.path.join(sessiondir, x) for x in os.listdir(sessiondir)]
    nodedirs = [x for x in direntries if stat.S_ISDIR(os.stat(x).st_mode)]
    nodedirs = [x for x in nodedirs if os.path.exists(os.path.join(x, "server"))]
    if filterfunc:
        nodedirs = list(filter(filterfunc, nodedirs))
    nodedirs.sort()
    return [clientcls(x) for x in nodedirs]
